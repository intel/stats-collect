...
"""make-a-release - make a new 'stats-collect' project release."""

# TODO:
#   - Make version handling like in pepc and remove the 'ToolInfo.py' file.
#   - Make man page handling like in pepc.
...
if typing.TYPE_CHECKING:
    ...
    class _CmdlineArgsTypedDict(typing.TypedDict, total=False):
        ...
        Attributes:
            src_path: Path to the 'stats-collect' project git repository to make a release from
...
# Configure the root 'main' logger, not the child 'main.stats-collect', so that messages from
# pepctools ('main.pepc.*') are also visible (e.g. 'ReleaseHelpers' push instructions).
...
def _build_arguments_parser() -> ArgParse.ArgsParser:
    ...
    text = f"""{_TOOLNAME} - make a new 'stats-collect' project release."""
    parser = ArgParse.ArgsParser(description=text, prog=_TOOLNAME, ver=_VERSION)

    text = """Path to the 'stats-collect' project git repository to make a release from
              (default is the current directory)."""
...
def _get_cmdline_args(args: argparse.Namespace) -> _CmdlineArgsTypedDict:
    ...
    if args.skip:
        ...
        valid_skip: set[_SkippableStepsType] = {
            "branch-check", "remotes-check", "uncommitted-check"
        }
...
def _regenerate_man_pages(src_path: Path) -> list[Path]:
    """
    Regenerate stats-collect man pages from the markdown source files in 'docs/man/'.

    Args:
        src_path: Path to the 'stats-collect' project git repository.
    ...
    man_dir = src_path / "statscollectdata" / "man" / "man1"
...
def _main(cmdl: _CmdlineArgsTypedDict):
    ...
    try:
        ...
        ReleaseHelpers.update_tool_version(src_path / "statscollecttools" / "ToolInfo.py",
                                           upcoming_version)
        modified.append(src_path / "statscollecttools" / "ToolInfo.py")

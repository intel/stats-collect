...
class Runner(ClassHelpers.SimpleCloseContext):
    ...
    def _add_label(self, label_json: str):
        """
        Parse a label JSON string and add the label to the statistics collector.

        Args:
            label_json: A JSON string representing the label to be added.
        """
        ...
        try:
            ...
            raise ErrorBadFormat(f"Failed to parse label JSON {label_json}:\n{errmsg}") from err

        name = label.get("name")
        if not name:
            raise ErrorBadFormat(f"No label name JSON '{label_json}'")
        if len(name) > MDCBase.MAX_KEY_LEN:
            raise ErrorBadFormat(f"Label name '{name}' is too long ({len(name)}, while max. is "
                                 f"{MDCBase.MAX_KEY_LEN} characters)")
        ...
        if name == "wlinfo":
            ...
            if metrics:
                self._stcoll.add_label(name, metrics=metrics)
            else:
                self._stcoll.add_label(name)

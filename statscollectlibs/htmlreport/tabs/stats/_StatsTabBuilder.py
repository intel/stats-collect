...
class StatsTabBuilder:
    ...
    def _init_tab_bldrs(self):
        ...
        # 'IPMITabBuilder' is used for inband and out-of-band IPMI collection so has two statistic
        # names.
        for stname in _IPMITabBuilder.IPMITabBuilder.stnames:
            tab_builders[stname] = _IPMITabBuilder.IPMITabBuilder

        # TODO: this is an ugly hack to avoid initializing IPMI tab builder twice. This happens when
        # there is a mix of inband and out of band stats: the tab builder initialized 2 times, so
        # builds dataframe 2 times, double work. The proper way to fix it is to call both flavors
        # just "ipmi", instead of "ipmi-inband" and "ipmi-oob" in the info.yml file.
        aliases = {"ipmi-inband": False, "ipmi-oob": False}
        ...
        for stname in tab_builders:
            ...
            if stname in aliases:
                for intialized  in aliases.values():
                    if intialized:
                        supported_stnames.discard(stname)
                aliases[stname] = True
